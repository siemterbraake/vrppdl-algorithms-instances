...
bool Solution::isValid(bool destroyed) const {
    /*
    ...
    */
    bool valid = true;
    float tol = 5; // Tolerance for float comparisons
    float costRoutes = 0; // Cost based on the sum of all route costs
    ...
    std::size_t nVisits = 0; // Number of visits
    int timeDriveTot = 0; // Total travel time
    std::vector<std::size_t> visitedCustNodes; // Set to fill with visited nodes
    ...
    for (std::size_t i = 0; i < d_pInst->d_nSL; i++) {
        ...
        std::size_t qSL = 0; // Quantity on a scheduled line
        for (std::size_t j = 0; j < d_SLSols[i].d_nAssigned; j++) {
    ...
    for (std::size_t i = 0; i < d_pInst->d_nDepots; i++) {
        ...
        for (std::size_t j = 0; j < d_depotSols[i].d_nVehicleTypes; j++) {
            ...
            if (curVehTypeSol.d_nUsedVehicles > curVehTypeSol.d_nVehicles) {
                if (curVehTypeSol.d_penalty != static_cast<float>(curVehTypeSol.d_nUsedVehicles - curVehTypeSol.d_nVehicles) * d_pSettings->d_excessiveRoutePenalty) {
                    std::cout << "Penalty for the excessive number of vehicles is incorrect\n";
                    return valid = false;
                }
            ...
            for (std::size_t k = 0; k < curVehTypeSol.d_nUsedVehicles; k++) {
                ...
                for (std::size_t l = 1; l < curRoute.d_nVisits; l++) {
                    ...
                    if (d_pInst->d_nodes[curVisit.d_idNode].d_idDepot != i && d_pInst->d_nodes[curVisit.d_idNode].d_idDepot < d_pInst->d_nDepots) {
                        if (d_visitLocs.at(curVisit.d_idNode).d_idSL == std::numeric_limits<std::size_t>::max()) {
                ...
                if (std::abs(routeDist - curRoute.d_dist) > tol) {
            ...
            costDist += distVeh*d_pInst->d_vehTypes[j].d_costVariable+
                        static_cast<float>(curVehTypeSol.d_nUsedVehicles)*d_pInst->d_vehTypes[j].d_costFixed;
        }
    }
    ...
    if (std::abs(costDist + costSL + costPenalty - d_cost) > tol) {
        std::cout << "Distance-based calculation is off cost-based calculation by " <<  std::abs(costDist + costSL + costPenalty - d_cost) <<"\n";
        return valid = false;
    }
    // Verify that the cost of the depot solutions is equal to the cost of the solution
    if (std::abs(costRoutes + costSL + costPenalty - d_cost) > tol) {
        std::cout << "Cost of depot solutions is not equal to cost of solution\n";
        return valid = false;
    }
    std::set<std::size_t> visitedCustNodesSet(visitedCustNodes.begin(), visitedCustNodes.end());
    // Verify that the number of unique nodes is equal to the number of nodes
    if (!destroyed && visitedCustNodesSet.size() + d_reqInfeasible.size() != d_pInst->d_nCustNodes) {
    ...
    for (std::size_t i = 0; i < d_pInst->d_nDepots; i++) {
        ...
        for (std::size_t j = 0; j < d_pInst->d_nVehTypes; j++) {
            ...
            for (std::size_t k = 0 ; k < d_depotSols[i].d_vehTypeSols[j].d_nUsedVehicles; k++) {
                ...
                // Iterate over visits
                for (std::size_t l = 1; l < curRoute.d_nVisits-1; l++) {
