...
Route::Route() {
    ...
    */
    d_idRoute = std::numeric_limits<std::size_t>::max();
...
Route::Route(std::size_t idRoute, std::size_t idVehicleType, std::size_t idDepot) : 
...
std::pair<std::size_t,float> Route::checkGreedyInsert(const Node &req, int twDepotStart,
    ...
    */
    ...
    std::size_t iStart = d_iMix + 1, iEnd = d_nVisits - 1;
    if (req.d_type == 1) {
    ...
    for (std::size_t i=iStart;i<=iEnd;i++) {
        ...
        float distToReq = graph.getDist(vehType.d_idVehicleType,d_visits[i-1].d_idNode,req.d_idNode);
        float distFromReq = graph.getDist(vehType.d_idVehicleType,req.d_idNode,d_visits[i].d_idNode);
        float distInsert = distToReq + distFromReq - graph.getDist(vehType.d_idVehicleType,d_visits[i-1].d_idNode,d_visits[i].d_idNode);
        ...
        if (checkCapacity) {
            ...
            if (req.d_type == 0) {
                ...
                for (std::size_t j=d_nVisits-1;j>=i;j--) {
        ...
        if (timeArrivalReq > req.d_twCustEnd) {
...
void Route::insert(const Node &req, std::size_t pos, int twDepotStart,
    ...
    */
    ...
    float distToReq = graph.getDist(vehType.d_idVehicleType,d_visits[pos-1].d_idNode,req.d_idNode);
    float distFromReq = graph.getDist(vehType.d_idVehicleType,req.d_idNode,d_visits[pos].d_idNode);
    float distRem = graph.getDist(vehType.d_idVehicleType,d_visits[pos-1].d_idNode,d_visits[pos].d_idNode);
    float distInsert = distToReq+distFromReq-distRem;
    ...
    d_dist += distInsert;
    d_cost += distInsert*vehType.d_costVariable;

    std::size_t qDel = 0, qCol = 0;
    ...
    if (req.d_type == 0) {
       // Case of a collection
        ...
        for (std::size_t i=pos;i<d_nVisits-1;i++) {
    ...
    int twSlackEnd = req.d_twCustEnd-timeArrivalReq;
    ...
    for (std::size_t i=pos+1;i<d_nVisits;i++) {
...
void Route::remove(const Node &req, std::size_t pos, const Graph &graph, const VehicleType &vehType, 
    ...
    */
    ...
    float distToReq = graph.getDist(vehType.d_idVehicleType,d_visits[pos-1].d_idNode,d_visits[pos].d_idNode);
    float distFromReq = graph.getDist(vehType.d_idVehicleType,d_visits[pos].d_idNode,d_visits[pos+1].d_idNode);
    float distNewEdge = graph.getDist(vehType.d_idVehicleType,d_visits[pos-1].d_idNode,d_visits[pos+1].d_idNode);
    float distRemove = distToReq+distFromReq-distNewEdge;
    ...
    if (req.d_type == 1 && d_visits[0].d_timeDeparture != 0) {
        ...
        if (visitLocs.at(req.d_idNode).d_idSL != std::numeric_limits<std::size_t>::max()) {
        ...
        if (d_visits[0].d_timeDeparture == timeMinDepartureReq) {
            ...
            for (std::size_t i=1;i<d_nVisits-1;i++) {
                if (i==pos || d_visits[i].d_type == 0) {
                    // Skip the removed position
                    continue;
                }
                if (visitLocs.at(d_visits[i].d_idNode).d_idSL != std::numeric_limits<std::size_t>::max()) {
                    // If the request is on a scheduled line, the arrival time of the SL is the min required departure time
...
void Route::updateLoadHelpers(const VehicleType &vehType) {
    ...
    */
    std::size_t newMaxLoad = 0;
    std::size_t newIFill = d_nVisits-1, newIMix = 1;
    bool iFillPassed = false, iMixPassed = false;
    for (std::size_t i=0;i<d_nVisits;i++) {
    ...
    d_iFill = newIFill;
    d_iMix = std::max(newIMix,std::size_t(0));
}